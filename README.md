...
### Self-service security, delivered natively — one Organization at a time

---

## Abstract

**VMware vDefend** is the security suite built directly into VMware Cloud Foundation (VCF) 9 — Distributed Firewall micro-segmentation, VPC Gateway Firewall, and Transit Gateway Firewall, enforced at every layer around a workload. What makes it more than "another firewall" is how it's delivered: VCF Automation's **Cloud Consumption Interface (CCI)** exposes vDefend's full security model as declarative, Kubernetes-style custom resources, scoped per **Organization (tenant)** through ordinary namespace and RBAC boundaries. A provider admin sets the guardrails once — quotas, regions, role bindings — and from that point on, every tenant Organization gets its own self-service security surface: group membership, firewall policy, and posture, provisioned and iterated on without a single ticket to a central security team. This paper explains what vDefend is, how VCF Automation and its APIs deliver that self-service model in a true multi-tenant environment, and — because these are just Kubernetes objects — how to operationalize it with **Terraform** and **Argo CD**, including a complete, runnable multi-tier tenant onboarding example.

## 1. Executive Summary

Traditionally, configuring per-tenant network security in a private cloud meant a security admin logging into a management console, building Groups and DFW/gateway firewall rules by hand for every tenant, and hoping change control caught drift. That model doesn't scale with self-service infrastructure, and it's the single most common reason security becomes the long pole in an app team's deployment timeline — the compute and network layers are self-service, but security stays centralized and manual.

**vDefend removes that bottleneck by design.** VCF Automation's CCI projects vDefend's entire security model — group membership, firewall policies, and firewall rules at three distinct enforcement points — as Kubernetes custom resources scoped to a **Supervisor Namespace** inside a tenant **Project (Organization)**. Each Organization gets its own API surface for its own security posture, walled off from every other tenant by the same RBAC and quota mechanisms that already govern its compute and network self-service. And because these are ordinary Kubernetes objects behind a standard API server, every mainstream infrastructure-as-code and GitOps tool works against them out of the box — no bespoke integration, no proprietary SDK, no waiting on a vendor connector.

The five resource kinds this paper focuses on map directly onto vDefend's three real enforcement points inside and around a tenant's VPC:
...
- **Reuse what already works.** No proprietary automation surface — the same Terraform and Argo CD skills a platform team already has apply directly, on day one.

Once that self-service API surface exists, operationalizing it is a matter of tooling, not new infrastructure. This paper shows two complementary automation levers built on top of it:
