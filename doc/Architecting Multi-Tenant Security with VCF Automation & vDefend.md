...
VCFA maps tenant abstractions onto native vSphere, VCF Networking (NSX) and Security (vDefend) constructs, using the vSphere Supervisor as its control plane and translation engine. A tenant never provisions resources by hand — they declare intent against VCFA, and the Supervisor renders it into the fabric objects underneath. A provider admin establishes top-level administrative boundaries, while tenant users self-service consumable resources underneath within defined RBAC and quotas:
...
| **Tenant Tier** | **Application Owner / Developer** | Application workloads (VMs, VKS Kubernetes clusters, Data Services), standard workload labels inside assigned namespaces. | Cannot assign or remove protected labels, bypass namespace boundaries, or override VPC and/or Namespace security postures set above them. |

**Context Scoping for Human and Machine Identities**

Each persona receives a kubeconfig context scoped strictly to the specific Organization and Projects permitted by their ProjectRoleBinding (authorization.cci.vmware.com). Consequently, the same token that authorizes a developer to deploy a workload also acts as the boundary constraining their visibility across the platform. Resources outside a persona's assigned scope are neither writable nor discoverable through API enumeration — a key requirement that allows platform providers to safely expose standard Kubernetes API endpoints directly to tenants.
...
kubectl label virtualmachines prod01-vm02 protected/app01=frontend
```
...
The paired `FirewallPolicy` layers cleanly — intra-app allow, a specific inbound allow, then a drop-all lockdown:
...
Mapping metadata directly to policy groups transforms security into an intrinsic step of the workload lifecycle, eliminating manual, post-launch policy configuration. Pairing a `NetworkSecurityGroup` selecting `protected/env: dev` with a `FirewallPolicy` provisioned *before* any workload exists ensures the workload immediately joins its respective security group and inherits its firewall policy the moment it powers on. Because these groups and policies are pre-provisioned during environment onboarding, there is zero exposure window between creation and protection.
...
**Scenario**:  
Tenant **acme** secures its `dev01` vSphere Namespace in `dev-vpc` using a layered defense-in-depth model:

* **VPC Perimeter:** The `vpc-isolation-with-essential-services` Security Profile blocks all external ingress/egress except core services like DNS, DHCP and NTP.  
* **Namespace Baseline:** `dev01` enforces a default-deny policy that allows only internal namespace traffic and incoming HTTPS.  
* **App Ringfencing (**`app01`**):** The application layer is microsegmented further—permitting intra-app communication, inbound HTTPS from outside `dev01`, while dropping all other traffic by default.

### 6.1 Terraform Provider Chain
...
Every VPC gets a `SecurityProfileAttachment` automatically when it's provisioned, defaulted to the fully-open `none` strategy (§5.1) — so locking `dev-vpc` down to `vpc-isolation-with-essential-services` means importing and patching that existing object rather than creating a new one.
...
The following Terraform script creates a `dev01_namespace_group` Group defined with dynamic matching criteria. Instead of maintaining static member lists, it dynamically groups all workloads provisioned within the dev01 vSphere Namespace as they scale up or down. The resource then attaches a baseline segmentation FirewallPolicy to this group, enforcing the namespace's default security posture:

- Intra-Namespace (East-West): Allowed with `JumpToApplication` category policies and rules.  
...
The following configuration establishes the `app01-ringfencing` policy (§5.3). By leveraging dynamic label matching (`protected/app01`) over static IP management, the Application-category policy ringfences `app01`—allowing only internal application traffic, HTTPS ingress from sources different than `dev01` namespace and blocking all other connections.
...
**Inverted Source Matching:**  
Setting `sourcesExcluded = true` negates the `from` block on `allow-https-inbound`, effectively permitting HTTPS traffic from any source *except* the `dev01-namespace` group. This keeps HTTPS ingress open to external and cross-namespace callers while blocking local development workloads.

### 6.5 Day-2, Tenant-Driven Change

To enable cross-namespace metrics scraping from the monitoring VPC to `app01`, a Day-2 Terraform update introduces a `monitoring01` group and prepends an Allow rule prior to the default inbound drop rule in the `namespace_segmentation_dev01` Environment policy. This permits targeted monitoring traffic directly, bypassing downstream Application-tier policy modifications.
...
VCF multi-tenant security delivered by vDefend is built on layered, composable boundaries—from Organization-level perimeter controls down to microsegmented applications. Whether delivered as a provider-managed security posture or enabled for tenant self-service, all security constructs are Kubernetes-native CRDs managed via the VCFA Cloud Consumption API. This allows platform and application teams alike to drive end-to-end automation—from Day-0 baselines to Day-2 updates—using declarative Infrastructure-as-Code and GitOps workflows like Terraform, Argo CD, or standard Kubernetes manifests.

Embedding vDefend enforcement directly into an IaC pipeline turns security from a manual gatekeeper into an intrinsic platform capability. Ultimately, consuming vDefend as code removes traditional operational bottlenecks—transforming security into an ambient, automated property of the platform that drives faster time-to-market and continuous compliance.
...
| **Argo CD** | The GitOps controller §7 discusses as an optional, continuous day-2 policy reconciliation pattern |
